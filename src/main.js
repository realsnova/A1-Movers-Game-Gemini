...
const Game = {
    ...
    preWarmAudio() {
        ...
        try {
            ...
            // Unlock SpeechSynthesis for mobile Safari/Chrome
    ...
    startSpeechRecognition(wordId) {
        if (navigator.userAgent.match('CriOS')) {
            this.ui.showModal("不支援的瀏覽器", "蘋果 (Apple) 限制了 iOS 版 Chrome 的語音辨識功能。\\n\\n💡 請改用 **Safari 瀏覽器** 開啟本遊戲，即可正常使用語音輸入功能！");
            return;
        }

        const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SpeechRecognition) {
            this.ui.showModal("不支援", "您的瀏覽器不支援語音辨識，建議使用 Chrome 或 Edge 瀏覽器。");
            return;
        }
